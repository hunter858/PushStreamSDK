...
static void aw_create_x264_param(aw_x264_context *aw_ctx, x264_param_t ** param){
    ...
    x264_param->i_csp = aw_ctx->config.input_data_format;
...
//编码一帧数据
extern void aw_encode_yuv_frame_2_x264(aw_x264_context *aw_ctx, int8_t *yuv_frame, int y_stride, int len) {
    if (len > 0 && yuv_frame) {
        int actual_width = y_stride;
        //数据保存到pic_in中
        ...
        //编码
        int result = x264_encoder_encode(aw_ctx->x264_handler, &aw_ctx->nal, &aw_ctx->nal_count, aw_ctx->pic_in, aw_ctx->pic_out);
        
        if (result < 0) {
            printf("x264 encoder error errorCode:%d \n",result);
        }
